...
use Pimcore\Model\DataObject\Operation;
...
class TechMapSubmitFormType extends AbstractType
{
    /**
     * @inheritDoc
     */
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $symbolsChoices = [];
        foreach ($symbols as $symbolA) {
            $symbolsChoices[$symbolA->getName()] = $symbolA->getId();
        }

        ksort($symbolsChoices);

        $operationChoices = [];
        foreach ($operations as $op) {
            $operationChoices[$op->getName()] = $op->getName();
        }
        ksort($operationChoices);

        $equipmentChoices = [] ;
        foreach ($equipments as $equipment) {
            $equipmentChoices[$equipment->getName()] = $equipment->getId();
        }
        ksort($equipmentChoices);
